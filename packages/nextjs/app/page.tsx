...
const Starknet: NextPage = () => {
  ...
  const [isTransactionPending, setTransactionPending] = useState<string | null>(null);
      ...
      <div className="container mx-auto mt-10 px-4">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4 mb-12">
          {[{
            ...
            <div key={index} className="bg-gradient-to-br from-gray-700 via-gray-800 to-gray-900 text-white p-6 rounded-lg shadow-lg border border-gray-600">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <img src={token.logo.src} alt={`${token.name} Icon`} className="w-12 h-12 object-contain" />
                  <p className="text-lg font-semibold">{token.name}</p>
                </div>
                <p className="text-lg font-semibold">
                  {token.balance ? `${formatEther(Number(token.balance))} ${token.name}` : `0 ${token.name}`}
                </p>
              </div>
              <div className="flex items-center justify-between mt-2">
                <p className="text-blue-400">{token.address ? <Address address={token.address as AddressType} /> : 'N/A'}</p>
                <button
                ...
                >
                  {isTransactionPending === token.name ? "Minting..." : `Mint ${token.name}`}
                </button>
              </div>
            </div>
          ))}
          ...
          <div className="relative flex justify-center items-center md:col-span-2 lg:col-span-4 my-12">
            ...
            >
              <div className={`absolute inset-0 flex items-center justify-center ${isTransactionPending === 'ALL' ? 'animate-spin' : ''}`}>
                <div className="relative w-full h-full flex items-center justify-center">
                  <div className="absolute w-full h-full flex items-center justify-center">
                    <img src={usdtLogo.src} alt="USDT Icon" className="w-12 h-12 absolute" style={{ top: '15%', left: '15%' }} />
                    <img src={daiLogo.src} alt="DAI Icon" className="w-12 h-12 absolute" style={{ top: '15%', right: '15%' }} />
                    <img src={strkLogo.src} alt="STRK Icon" className="w-12 h-12 absolute" style={{ bottom: '15%', left: '15%' }} />
                    <img src={naiLogo.src} alt="NAI Icon" className="w-12 h-12 absolute" style={{ bottom: '15%', right: '15%' }} />
                  </div>
                </div>
              </div>
              <span className="font-semibold relative z-10">Mint All Tokens</span>
              <div className="absolute inset-0 rounded-full border-0 border-transparent bg-gradient-to-br from-blue-600 to-purple-600" style={{ maskImage: 'linear-gradient(to right, rgba(0,0,0,0) 10%, rgba(0,0,0,0.5) 50%, rgba(0,0,0,0) 90%)' }}></div>
            </button>
          </div>
        </div>
      </div>
