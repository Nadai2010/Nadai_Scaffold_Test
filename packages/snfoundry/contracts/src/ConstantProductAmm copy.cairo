...
pub trait IConstantProductAmm<TContractState> {
    ...
    fn swap_volatil(ref self: TContractState, token_in: ContractAddress, amount_in: u256) -> u256;
    fn add_liquidity(ref self: TContractState, amount0: u256, amount1: u256) -> u256;
    fn add_liquidity_volatil(ref self: TContractState, amount2: u256, amount3: u256) -> u256;
    fn remove_liquidity(ref self: TContractState, shares: u256) -> (u256, u256);
    fn remove_liquidity_volatil(ref self: TContractState, shares2: u256) -> (u256, u256);
    fn get_reserves(self: @TContractState) -> (u256, u256);
    fn get_reserves_volatil(self: @TContractState) -> (u256, u256);
    fn get_total_supply(self: @TContractState) -> u256;
    fn get_balance_of(self: @TContractState, owner: ContractAddress) -> u256;
}
...
mod ConstantProductAmm {
    ...
    struct Storage {
        ...
        token2: IERC20Dispatcher,
        token3: IERC20Dispatcher,
        reserve0: u256,
        reserve1: u256,
        reserve2: u256,
        reserve3: u256,
    ...
    #[constructor]
    fn constructor(
        ref self: ContractState, token0: ContractAddress, token1: ContractAddress, token2: ContractAddress, token3: ContractAddress, fee: u16
    ) {
        ...
        self.token2.write(IERC20Dispatcher { contract_address: token2 });
        self.token3.write(IERC20Dispatcher { contract_address: token3 });
        self.fee.write(fee);
    }
    ...
    impl PrivateFunctions of PrivateFunctionsTrait {
        ...
        fn _update2(ref self: ContractState, reserve2: u256, reserve3: u256) {
            self.reserve2.write(reserve2);
            self.reserve3.write(reserve3);
        }
            ...
            assert(
                token == self.token0.read().contract_address
                    || token == self.token1.read().contract_address
                         || token == self.token2.read().contract_address
                            || token == self.token3.read().contract_address,
                'invalid token'
            );
            token == self.token0.read().contract_address
        }
    ...
    impl ConstantProductAmm of super::IConstantProductAmm<ContractState> {
        ...
        fn swap_volatil(ref self: ContractState, token_in: ContractAddress, amount_in: u256) -> u256 {
            assert(amount_in > 0, 'amount in = 0');
            let is_token2: bool = self.select_token(token_in);

            let (token2, token3): (IERC20Dispatcher, IERC20Dispatcher) = (
                self.token2.read(), self.token3.read()
            );
            let (reserve2, reserve3): (u256, u256) = (self.reserve2.read(), self.reserve1.read());
            let (
                token_in, token_out, reserve_in, reserve_out
            ): (IERC20Dispatcher, IERC20Dispatcher, u256, u256) =
                if (is_token2) {
                (token2, token3, reserve2, reserve3)
            } else {
                (token3, token2, reserve3, reserve2)
            };
            ...
            self._update2(self.token2.read().balance_of(this), self.token3.read().balance_of(this));
            amount_out
        }
        ...
        fn add_liquidity_volatil(ref self: ContractState, amount2: u256, amount3: u256) -> u256 {
            let caller = get_caller_address();
            let this = get_contract_address();
            let (token2, token3): (IERC20Dispatcher, IERC20Dispatcher) = (
                self.token2.read(), self.token3.read()
            ...
            token2.transfer_from(caller, this, amount2);
            token3.transfer_from(caller, this, amount3);

            let (reserve2, reserve3): (u256, u256) = (self.reserve2.read(), self.reserve1.read());
            if (reserve2 > 0 || reserve3 > 0) {
                assert(reserve2 * amount3 == reserve3 * amount2, 'x / y != dx / dy');
            }

            let total_supply = self.total_supply.read();
            let shares2 = if (total_supply == 0) {
                u256_sqrt(amount2 * amount3).into()
            } else {
                PrivateFunctions::min(
                    amount2 * total_supply / reserve2, amount3 * total_supply / reserve3
                )
            };
            assert(shares2 > 0, 'shares = 0');
            self._mint(caller, shares2);
        ...
        fn remove_liquidity_volatil(ref self: ContractState, shares2: u256) -> (u256, u256) {
            let caller = get_caller_address();
            let this = get_contract_address();
            let (token2, token3): (IERC20Dispatcher, IERC20Dispatcher) = (
                self.token2.read(), self.token3.read()
            ...
            let (bal2, bal3): (u256, u256) = (token2.balance_of(this), token3.balance_of(this));

            let total_supply = self.total_supply.read();
            let (amount2, amount3): (u256, u256) = (
                (shares2 * bal2) / total_supply, (shares2 * bal3) / total_supply
            );
            assert(amount2 > 0 && amount3 > 0, 'amount2 or amount3 = 0');

            self._burn(caller, shares2);
            self._update2(bal2 - amount2, bal3 - amount3);

            token2.transfer(caller, amount2);
            token3.transfer(caller, amount3);
            (amount2, amount3)
        }
        ...
        fn get_reserves_volatil(self: @ContractState) -> (u256, u256) {
            (self.reserve2.read(), self.reserve3.read())
        }
        ...
        fn get_balance_of(self: @ContractState, owner: ContractAddress) -> u256 {
